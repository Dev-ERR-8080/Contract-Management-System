{{-- resources/views/solar_contracts/index.blade.php --}}
@extends('layouts.app')
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-semibold text-gray-900">☀️ Solar Contracts</h1>
        <a href="{{ route('solar_contracts.create') }}" class="inline-block bg-blue-600 text-white text-sm px-6 py-3 rounded-md hover:bg-blue-700 shadow-md transition">+ Add Contract</a>
    </div>
    <!-- search box-->
    <form method="GET" action="{{ route('solar_contracts.index') }}" class="mt-4">
        ...
        <div>
            @include('solar_contracts.calendar')
        </div>
    </form>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach($contracts as $contract)
                    <tr class="hover:bg-gray-50">
                        ...
                        class="inline-block px-3 py-1 bg-green-100 text-green-700 rounded-full hover:bg-green-600 hover:text-white transition height-[20px]">
                            👁️ View
                        </a>

                        <a href="{{ route('solar.quotes.create', $contract->_id) }}"
                        class="inline-block px-3 py-1 bg-yellow-100 text-yellow-700 rounded-full hover:bg-yellow-500 hover:text-white transition height-[5px]">
                            💬 Quotes
                        </a>
                    </td>
